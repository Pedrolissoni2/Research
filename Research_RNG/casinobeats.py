...
def start_casinobeats_reports(days, key_words, date_limit, path_output, url_list, stats, proxies=None):
    ...
    while True:
        print(page)
        obj_bs4 = request_url(f"https://casinobeats.com/page/{page}/?s", proxies=proxies)
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output, proxies):
    ...
    news = obj_bs4.select("div.td-ss-main-content h3.entry-title.td-module-title a")
    # categories = obj_bs4.select("div.td-ss-main-content a.td-post-category")
    dates = obj_bs4.select("div.td-ss-main-content time.entry-date.updated.td-module-date")
    in_limit = True
    ...
    next_page = obj_bs4.select_one('a[aria-label="next-page"]')
...
def process_response_details(url, key_words, proxies):
    ...
    category_soup = obj_bs4_details.select('a.entry-crumb')
    if category_soup:
        category = category_soup[-1].text.strip()
    else:
        category = ""
        
    news_details_texts = obj_bs4_details.select("div.td-post-content.tagdiv-type p")
    for news_details in news_details_texts:
        news_details_text = news_details.text.strip()
