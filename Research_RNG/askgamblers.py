...
def start_askgamblers_reports(days, key_words, date_limit, path_output, url_list, stats, proxies=None):
    data_list = []
    url = "https://www.askgamblers.com/gambling-news/"
    page = 1
    while True:
        print(page)
        obj_bs4 = request_url(url, proxies=proxies)
        next_url, has_next_page = process_response(obj_bs4, date_limit, key_words, url_list, data_list, path_output, proxies=proxies)
        if has_next_page and next_url:
            url = next_url
            page = page + 1
        else:
            break
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output, proxies):
    news = obj_bs4.select("li.card-list__item a.card")
    ...
    next_page = obj_bs4.select_one("a.site-pagination__next")

    if next_page is not None and in_limit == True:
        return next_page["href"], True
    return None, False
