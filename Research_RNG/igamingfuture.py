...
def start_igamingfuture_reports(days, key_words, date_limit, path_output, url_list, stats):
    data_list = []
    page = 1
    while True:
        print(page)
        obj_bs4 = request_url(f"https://igamingfuture.com/search/?sf_paged={page}/")
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output):
    ...
    news = obj_bs4.select("a.cl-element-title__anchor")
    # categories = obj_bs4.select("h1.category-heading.other-categories-text")
    dates = obj_bs4.select("div.cl-element-published_date")
    in_limit = True
    # for new, category, date in zip(news, categories, dates):
    for new, date in zip(news, dates):
        news_date_str = date.text.strip()
        news_date_str = re.sub(r"(st|nd|rd|th)", "", news_date_str)
    ...
    next_page = obj_bs4.select_one("a.next.page-numbers")
...
def process_response_details(url, key_words):
    ...
    news_details_texts = obj_bs4_details.select("div.lc-block.pt-2.mt-1 p")
