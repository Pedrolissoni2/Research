...
def start_the_gamblest_reports(days, key_words, date_limit, path_output, url_list, stats):
    data_list = []
    page = 1
    while True:
        print(page)
        obj_bs4 = request_url(f"https://www.thegamblest.com/latest-gambling-news/page/{page}/")
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output):
    ...
    news = obj_bs4.select("p.news-title.h4 a")
    # categories = obj_bs4.select("ul.news-cat.list-style li:nth-of-type(1) a")
    dates = obj_bs4.select("ul.news-metainfo.list-style time")
    in_limit = True
    ...
    next_page = obj_bs4.select_one("i.fal.fa-chevron-right")
...
def process_response_details(url, key_words):
    ...
    category = obj_bs4_details.select_one("ul.news-cat.list-style li:nth-of-type(1) a")
    news_details_texts = obj_bs4_details.select("section.post-para p")
