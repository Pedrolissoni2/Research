...
def request_api(page):
    url = f"https://igamingbrazil.com/en/wp-json/wp/v2/posts?per_page=100&page={page}&_fields=id,title,date,link"
    print(url)
...
def process_response(posts, date_limit, key_words, url_list, data_list, path_output):
    for post in posts:
        ...
        if news_date >= date_limit:
            ...
            has_keyword, detail_title, category = process_response_details(new_url, key_words)
            title = detail_title if detail_title else api_title
            data_json = {
                "website": "ig_brazil",
                "category": category,
                "date": formatted_date,
...
def process_response_details(url, key_words):
    ...
    try:
        ...
        print(f"  [WARN] ig_brazil detail failed for {url}: {e}")
        title = ""
        category = ""
