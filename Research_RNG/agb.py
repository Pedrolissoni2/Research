...
def start_agb_reports(days, key_words, date_limit, path_output, url_list, proxies=None):
    data_list = []
    page = 1
    while True:
        print(page)
...
def request_api(url, page, proxies):
    ...
    sitecontent = requests.post(url, data=payload, proxies=proxies, headers=headers, verify=False)
    print(sitecontent.content)
