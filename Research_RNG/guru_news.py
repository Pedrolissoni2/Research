...
def start_guru_news_reports(days, key_words, date_limit, path_output, url_list, stats):
    data_list = []
    page = 1
    while True:
        print(page)
        obj_bs4 = request_url(f"https://casino.guru/news/latest/{page}")
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output):
    ...
    news = obj_bs4.select("div.results-main-content div.news-card a.news-card-header")
    categories = obj_bs4.select("div.results-main-content div.news-card-topics.mt-xs a.news-card-topic")
    dates = obj_bs4.select("div.results-main-content span.inline-flex.align-center.mr-s.time-ago")
    in_limit = True

    for new, category, date in zip(news, categories, dates):
        # for new, date in zip(news, dates):
        news_date_str = date['data-content']
        news_date = datetime.strptime(news_date_str, "%d %b %Y")
        formatted_date = news_date.strftime("%Y-%m-%d %H:%M:%S")

        if news_date >= date_limit:
            has_keyword = process_response_details(new["href"], key_words)
            data_json = {
                "website": "guru_news",
    ...
    next_page = obj_bs4.select_one("a.paging-num.paging-right.js-paging-item")
...
def process_response_details(url, key_words):
    ...
    news_details_texts = obj_bs4_details.select("div.news-detail-content.typography p")
