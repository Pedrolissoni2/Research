...
def start_cgn_reports(days, key_words, date_limit, path_output, url_list, stats):
    data_list = []
    page = 1
    while True:
        print(page)
        obj_bs4 = request_url(f"https://www.canadiangamingbusiness.com/casino/page/{page}/")
...
def process_response(obj_bs4: BeautifulSoup, date_limit, key_words, url_list, data_list, path_output):
    ...
    news = obj_bs4.select("h3.title a")
    categories = obj_bs4.select("span.h-text")
    dates = obj_bs4.select("span.time time")
    in_limit = True
...
def process_response_details(url, key_words):
    ...
    news_details_texts = obj_bs4_details.select("div.entry-content.clearfix.single-post-content p")
