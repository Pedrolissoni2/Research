...
RSS_BASE = "https://sbcnews.co.uk/feed/"
...
def start_sbcnews_reports(days, key_words, date_limit, path_output, url_list, stats, proxies=None):
    ...
    while True:
        ...
        log.info("[sbcnews] fetching RSS page %d", page)
    ...
    stats.append({"source": "SBCNews", "news_count": len(data_list)})
    return stats
...
def _fetch_detail(url, key_words, proxies):
    ...
    cat_tags = soup.select('span.post-cats a')
    category = cat_tags[-1].text.strip() if cat_tags else ""

    paras = soup.select("div.entry p")
